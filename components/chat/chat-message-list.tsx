...
export function ChatMessageList({ messages, onOpenCitation }: { messages: DashboardMessage[]; onOpenCitation: (citation: ChatCitation, scopedCitations: ChatCitation[], opener?: HTMLElement) => void }) {
  if (messages.length === 0) return <div className="empty-chat card-surface"><p className="panel-kicker">Korean-first academic career service</p><h2>무엇을 도와드릴까요?</h2><p>채용 공고, 마감일, 취업 프로그램을 한국어로 질문하면 확인된 출처와 수집일을 함께 보여드려요.</p></div>;
