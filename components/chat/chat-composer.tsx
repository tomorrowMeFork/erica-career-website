...
export function ChatComposer({ query, onQueryChange, onSubmit, isLoading, maxLength = 2000 }: { query: string; onQueryChange: (query: string) => void; onSubmit: () => void; isLoading: boolean; maxLength?: number }) {
  ...
  return (
    <form className="relative z-0 w-full min-w-0" onSubmit={(event) => { event.preventDefault(); if (!disabled) onSubmit(); }}>
      <Card className="erica-surface border-primary/20 py-4 ring-1 ring-primary/5">
        <CardContent className="grid gap-3 px-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end sm:px-5">
          <Textarea aria-label="질문 입력" value={query} maxLength={maxLength} className="min-h-24 resize-y border-primary/15 bg-white/90 text-base leading-7 focus-visible:border-primary" onChange={(event) => onQueryChange(event.target.value)} onKeyDown={(event) => {
