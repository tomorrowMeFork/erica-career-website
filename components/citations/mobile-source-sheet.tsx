...
export function MobileSourceSheet({ open, citations, selectedCitation, opener, onClose }: { open: boolean; citations: ChatCitation[]; selectedCitation?: ChatCitation; opener?: HTMLElement | null; onClose: () => void }) {
  ...
  return (
    <Sheet open={open} onOpenChange={(nextOpen) => { if (!nextOpen) onClose(); }}>
      <SheetContent side="bottom" showCloseButton={false} className="erica-surface-strong isolate z-[100] max-h-[88vh] gap-0 overflow-hidden rounded-t-2xl border-primary/25 p-0 text-card-foreground ring-1 ring-white/70" onCloseAutoFocus={(event) => { event.preventDefault(); restoreOpenerFocus(); }}>
        ...
        <ScrollArea className="erica-surface-muted max-h-[calc(88vh-104px)] px-4 py-4 pb-[calc(var(--space-4)+env(safe-area-inset-bottom))]">
          {selected !== undefined ? <SourceCard citation={selected} selected /> : <p className="erica-surface-muted rounded-lg border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">출처가 없어요.</p>}
        </ScrollArea>
      </SheetContent>
    </Sheet>
  );
