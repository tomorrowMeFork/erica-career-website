...
const approvedMethodMarkers: Readonly<
	Record<IngestionCollectionMethod, string>
> = {
...
export async function fetchApprovedText(
	...
	return fetchApproved(decision, url, options, "html", (response) =>
		response.text(),
	);
}
...
async function fetchApproved<T>(
	...
	const targetUrl = assertFetchAllowed(
		decision,
		url,
		options.approval_evidence_text,
		expectedKind,
	);
	...
	return limiter(async () => {
		...
		if (throttlingStatuses.has(response.status)) {
			throw new Error(
				`Approved fetch stopped on throttling-like status ${response.status} for ${targetUrl.href}`,
			);
		}

		if (!response.ok) {
			throw new Error(
				`Approved fetch stopped on non-2xx status ${response.status} for ${targetUrl.href}`,
			);
		}
		...
		if (!contentTypeMatches(contentType, expectedKind)) {
			throw new Error(
				`Approved fetch content-type mismatch for ${targetUrl.href}: ${contentType || "missing"}`,
			);
		}
...
function assertFetchAllowed(
	...
	const observedUrl = parseHttpsUrl(
		decision.observed_url,
		"approved observed URL",
	);
	const requiredDecisionMethod: IngestionCollectionMethod =
		expectedKind === "html" ? "public_html" : "manual_pdf_download";
	const requiredResponseType = expectedKind === "html" ? "html" : "pdf";

	if (decision.status !== "allowed") {
		throw new Error(
			`Approved fetch blocked: source decision for ${decision.source_id} is ${decision.status}`,
		);
	}

	if (
		decision.collection_method !== requiredDecisionMethod ||
		decision.response_type !== requiredResponseType
	) {
		throw new Error(
	...
	if (!approvalEvidenceText.includes(decision.source_id)) {
		throw new Error(
			`Approved fetch blocked: approval evidence does not name ${decision.source_id}`,
		);
	}
	...
	if (!approvalEvidenceText.includes(methodMarker)) {
		throw new Error(
			`Approved fetch blocked: approval evidence does not include ${methodMarker}`,
		);
	}

	if (!approvalEvidenceText.includes(observedUrl.href)) {
		throw new Error(
			`Approved fetch blocked: approval evidence does not include approved URL ${observedUrl.href}`,
		);
	}

	if (!isWithinObservedScope(targetUrl, observedUrl)) {
		throw new Error(
			`Approved fetch rejected off-scope URL before network: ${targetUrl.href}`,
		);
	}
...
function isWithinObservedScope(targetUrl: URL, observedUrl: URL): boolean {
	...
	const observedPrefix = observedUrl.pathname.endsWith("/")
...
function headersFor(expectedKind: ResponseKind): Headers {
	...
	headers.set(
		"User-Agent",
		"ERICA-Career-Chat-Capstone-Prototype/1.0 bounded-manual-sample",
	);
	headers.set(
		"Accept",
		expectedKind === "html"
...
function createTimeoutSignal(timeoutMs: number): AbortSignal {
	...
	globalThis.setTimeout(
		() => controller.abort(new Error("Approved fetch timeout elapsed")),
		timeoutMs,
	);
