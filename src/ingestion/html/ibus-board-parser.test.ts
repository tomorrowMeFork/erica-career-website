...
describe("parseIbusListingPage", () => {
	it("extracts titles and canonical official HTTPS detail URLs from the listing fixture", () => {
		...
		expect(entries[1]?.canonical_url).toBe(
			"https://ibus.hanyang.ac.kr/front/recruit/r-1/view?id=6467",
		);
	...
	it("rejects non-HTTPS or non-official detail URLs instead of hard-coding unsafe URLs", () => {
		...
		expect(() => parseIbusListingPage(unsafeHtml, boardUrl)).toThrow(
			"Rejected non-official ibus URL",
		);
	});
	...
	it("rejects same-host paths outside the exact approved ibus board/detail scope", () => {
		...
		expect(() => parseIbusListingPage(outOfScopeHtml, boardUrl)).toThrow(
			"Rejected out-of-scope ibus URL",
		);
		expect(() => parseIbusListingPage(prefixConfusionHtml, boardUrl)).toThrow(
			"Rejected out-of-scope ibus URL",
		);
	});

	it("ignores unapproved sibling board detail paths", () => {
		const r2Html = `<a href="/front/recruit/r-2/view?id=7001">r-2 blocked</a>`;

		expect(
			parseIbusListingPage(
				r2Html,
				"https://ibus.hanyang.ac.kr/front/recruit/r-2",
			),
		).toHaveLength(0);
	});
});
...
describe("parseIbusDetailPage", () => {
	it("extracts title, raw Korean text, cleaned text, posted date, and deadline evidence", () => {
		...
		expect(detail.title).toBe(
			"[채용시까지] ERICA 경상대학 현장실습 참여기업 모집",
		);
		expect(detail.title).not.toBe("123");
		expect(detail.raw_text).toContain(
			"ERICA 경상대학 학생을 대상으로 한 현장실습 참여기업 모집 안내입니다.",
		);
		expect(detail.cleaned_text).toContain(
			"공식 상세 URL: https://ibus.hanyang.ac.kr/front/recruit/r-1/view?id=6468",
		);
	...
	it("extracts live ibus body titles and full posted dates when title tags are generic", () => {
      ...
      본문 내용
		...
		expect(detail.title).toBe(
			"[한국에너지공단] 청년인턴(체험형 인턴) 채용 공고(~5/13)",
		);
	...
	it("builds schema-valid NormalizedRecord values with faculty-specific Korean metadata and citations", () => {
		const records = buildIbusNormalizedRecords({
			...
			detail_html_by_url: {
				...
				"https://ibus.hanyang.ac.kr/front/recruit/r-1/view?id=6467":
					detailHtml.replaceAll("채용시까지", "D-3"),
			},
			page_url: boardUrl,
		...
		expect(firstRecord.citation_anchors).toEqual([
			{
				url: detailUrl,
				label:
					"공식 취업정보 상세: [채용시까지] ERICA 경상대학 현장실습 참여기업 모집",
			},
		]);
	});
