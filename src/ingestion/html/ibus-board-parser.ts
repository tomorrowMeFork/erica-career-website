...
export function parseIbusListingPage(
	html: string,
	pageUrl = IBUS_BOARD_URL,
): IbusListingEntry[] {
...
export function parseIbusDetailPage(
	...
	const title = firstNonEmptyText([
		...
		$("h2").first().text(),
		extractIbusPostTitle(bodyText),
		$("title")
			.first()
			.text()
			.replace(/\s*-\s*취업정보\s*$/u, ""),
	]);
	...
	return {
		...
		...(classifiedDeadline.deadline_date
			? { deadline_date: classifiedDeadline.deadline_date }
			: {}),
	};
}

export function buildIbusNormalizedRecords(
	input: BuildIbusNormalizedRecordsInput,
): NormalizedRecord[] {
	...
	return listingEntries.map((entry) => {
		...
		const contentHash = sha256(
			[detail.cleaned_text, detail.canonical_url].join("\u001f"),
		);
...
function assertIbusBuildAllowed(
	...
	if (decision.collection_method !== "public_html") {
		failures.push(
			`source decision collection method is ${decision.collection_method}`,
		);
	}
	...
	if (!approvalEvidenceText.includes("approved_bounded_browser_discovery")) {
		failures.push(
			"approval evidence does not approve bounded browser discovery",
		);
	}
	if (!approvalEvidenceText.includes(IBUS_BOARD_URL)) {
		failures.push(
			"approval evidence does not include the original ibus seed URL",
		);
	}

	if (failures.length > 0) {
		throw new Error(
			`Ibus normalized record build blocked: ${failures.join("; ")}`,
		);
	}
}
...
function normalizeIbusUrl(href: string, baseUrl: string): string {
	...
	if (
		parsedUrl.pathname !== "/front/recruit/r-1" &&
		parsedUrl.pathname !== "/front/recruit/r-1/view"
	) {
...
function firstNonEmptyText(values: readonly string[]): string {
	return (
		values
			.map((value) => cleanBlockText(value))
			.find((value) => value.length > 0) ?? ""
	);
}

function extractIbusPostTitle(bodyText: string): string {
	const match = bodyText.match(/(?:^|\n)목록\n(.+?)\s+Hit\s+\d+/u);
	return match?.[1] ? cleanInlineText(match[1]) : "";
}

function extractPostedRawText(
	timeTexts: readonly string[],
	scopeText: string,
): string {
	const timeText = timeTexts.find(
		(value) => parseKoreanDateToIso(value) !== null,
	);
	if (timeText) {
	...
	const match = scopeText.match(
		/20\d{2}[.\-/년]\s*(?:1[0-2]|0?[1-9])[.\-/월]\s*(?:3[01]|[12]\d|0?[1-9])(?:일)?/u,
	);
