...
const ibusDetailUrl = "https://ibus.hanyang.ac.kr/front/recruit/r-1/view?id=6468";
const cdpPdfUrl = "https://cdp.hanyang.ac.kr/office/%EB%A7%A4%EB%89%B4%EC%96%BC_%ED%95%99%EC%83%9D.pdf";
...
const ibusApprovalEvidence = ["ibus-employment-board", "approved_bounded_browser_discovery", ibusUrl].join("\n");
const cdpPdfApprovalEvidence = ["cdp-student-guide-pdf", "approved_manual_download", cdpPdfUrl].join("\n");
...
describe("fetchApprovedText", () => {
  it("rejects an off-host URL before fetch is called", async () => {
    const fetchImpl = fetchMockReturning(new Response("unused", { headers: { "content-type": "text/html" } }));

    await expect(
      fetchApprovedText(ibusDecision, "https://evil.example/front/recruit/r-1", {
  ...
  it("rejects a blocked decision before fetch is called", async () => {
    ...
    await expect(
      fetchApprovedText({ ...ibusDecision, status: "blocked", reasons: ["held"] }, ibusUrl, {
        approval_evidence_text: ibusApprovalEvidence,
        fetch_impl: fetchImpl,
      }),
  ...
  it("fetches an approved synthetic ibus HTML URL exactly once", async () => {
    const fetchImpl = fetchMockReturning(new Response("<html>취업정보</html>", { headers: { "content-type": "text/html; charset=utf-8" } }));
  ...
  it("stops on HTML content-type mismatch", async () => {
    const fetchImpl = fetchMockReturning(new Response("{}", { headers: { "content-type": "application/json" } }));
  ...
  it("keeps same-host body reads inside the concurrency limiter", async () => {
    ...
    const fetchImpl = vi.fn<typeof fetch>(() => Promise.resolve(responses.shift() ?? new Response("unexpected")));
...
describe("fetchApprovedBytes", () => {
  it("fetches an approved synthetic CDP PDF decision exactly once", async () => {
    ...
    const fetchImpl = fetchMockReturning(new Response(pdfBytes, { headers: { "content-type": "application/pdf" } }));
  ...
  it("rejects a manual PDF download on an off-scope path before fetch", async () => {
    const fetchImpl = fetchMockReturning(new Response(new Uint8Array([1]), { headers: { "content-type": "application/pdf" } }));

    await expect(
      fetchApprovedBytes(cdpPdfDecision, "https://cdp.hanyang.ac.kr/office/other.pdf", {
        approval_evidence_text: cdpPdfApprovalEvidence,
        fetch_impl: fetchImpl,
      }),
    ).rejects.toThrow("off-scope URL before network");
  ...
  it("stops on throttling-like statuses", async () => {
    const fetchImpl = fetchMockReturning(new Response("slow down", { status: 429, headers: { "content-type": "application/pdf" } }));
